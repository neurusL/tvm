...
def has_flashinfer():
    ...
    try:
        from tvm.relax.backend.cuda import flashinfer  # pylint: disable=import-outside-toplevel
        return True
    except ImportError:
        return False
...
def compute_reference_grouped_gemm(
    ...
    result_fp32 = torch.cat(results, dim=0)  # [total_m, n]
...
def generate_test_data(
    ...
    assert batch_size == len(m_sizes), f"batch_size ({batch_size}) must equal len(m_sizes) ({len(m_sizes)})"
    ...
    a_fp32 = torch.randn(total_m, k, device=torch_device, dtype=torch.float32) * 0.1
    
    assert (dtype_b == "float8_e4m3fn")
    b_fp32 = torch.randn(batch_size, n, k, device=torch_device, dtype=torch.float32) * 0.1

    if scale_major_mode == "K": # K mode:
        # a_scale: (cum_m, k // block_size) 
        # b_scale: (batch_size, n // block_size, k // block_size)
        if scale_granularity_m == 1:
            scale_a_shape = (total_m, k // scale_granularity_k)
            tile_a_shape = (1, scale_granularity_k)
        else:
        ...
        tile_b_shape = (scale_granularity_n, scale_granularity_k)
        
    else:  # MN mode
        # a_scale: (k // block_size, cum_m)
        # b_scale: (batch_size, k // block_size, n // block_size)
        if scale_granularity_m == 1:
            scale_a_shape = (k // scale_granularity_k, total_m)
            tile_a_shape = (scale_granularity_k, 1)
        else:
            scale_a_shape = (k // scale_granularity_k, total_m // scale_granularity_m)
            tile_a_shape = (scale_granularity_k, scale_granularity_m)
            
        scale_b_shape = (batch_size, k // scale_granularity_k, n // scale_granularity_n)
        tile_b_shape = (scale_granularity_k, scale_granularity_n)

    # quantize A
    a_quantized, scale_a = quantize_fp8(a_fp32, scale_a_shape, tile_a_shape, scale_major_mode)
    a_dequant = dequantize_fp8(a_quantized, scale_a, scale_major_mode)
    ...
    for i in range(batch_size):
        b_single = b_fp32[i]  # Shape: (n, k)
        
        if scale_major_mode == "K":
            scale_b_single_shape = (n // scale_granularity_n, k // scale_granularity_k)
        else:
            scale_b_single_shape = (k // scale_granularity_k, n // scale_granularity_n)
            
        b_quantized, scale_b_single = quantize_fp8(
            b_single, scale_b_single_shape, tile_b_shape, scale_major_mode
        )
        b_dequant = dequantize_fp8(b_quantized, scale_b_single, scale_major_mode)
        b_quantized_list.append(b_quantized)
        b_dequant_list.append(b_dequant)

        scale_b_list.append(scale_b_single)
    
    # Stack the results
    b_quantized = torch.stack(b_quantized_list, dim=0)  # Shape: (batch_size, n, k)
    b_dequant = torch.stack(b_dequant_list, dim=0)

    scale_b = torch.stack(scale_b_list, dim=0)  # Shape: (batch_size, ..., ...)
    
    if dtype_a == "float8_e4m3fn":
        a_tvm = tvm.nd.array(a_quantized.view(torch.uint8).cpu().numpy().view(fp8_dtype), device=device)
    else:
        a_tvm = tvm.nd.from_dlpack(a_quantized)
    
    if dtype_b == "float8_e4m3fn":
        b_tvm = tvm.nd.array(b_quantized.view(torch.uint8).cpu().numpy().view(fp8_dtype), device=device)
    else:
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (128, 128, 128),    # Block-wise scaling
    ]
)
...
def test_grouped_gemm_correctness(
    dtype_a, dtype_b, dtype_out,
    scale_granularity_m, scale_granularity_n, scale_granularity_k,
    scale_major_mode, mma_sm, test_case
):
    ...
    # if dtype_a.startswith("float8") and mma_sm != 1:
    #     pytest.skip("FP8 only supports MMA_SM=1")
    ...
    try:
        ...
        print(test_data['b'].shape)
        print(test_data['scale_a'].shape)
        print(test_data['scale_b'].shape)
        print(output.shape)
        print(test_data['m_indptr'])

        
        
        result = grouped_gemm_fn(
        ...
        print("kernel call result", result)
        ...
        if not torch.allclose(output_torch, reference, rtol=rtol, atol=atol):
            max_diff = torch.max(torch.abs(output_torch - reference)).item()
            rel_diff = torch.max(torch.abs((output_torch - reference) / (reference + 1e-8))).item()
            
            pytest.fail(
                f"Numerical mismatch for {dtype_a}→{dtype_out}, "
                f"granularity=({scale_granularity_m},{scale_granularity_n},{scale_granularity_k}), "
                f"scale_mode={scale_major_mode}, mma_sm={mma_sm}\n"
                f"Max absolute difference: {max_diff}\n"
                f"Max relative difference: {rel_diff}\n"
                f"Expected tolerance: rtol={rtol}, atol={atol}"
            )
        
        print(f"Correctness test passed for {dtype_a}->{dtype_out}, "
              f"granularity=({scale_granularity_m},{scale_granularity_n},{scale_granularity_k}), "
              f"scale_mode={scale_major_mode}, mma_sm={mma_sm}, "
...
if __name__ == "__main__":
    ...
    if has_flashinfer() and has_cutlass():
        print("Running GroupedGemm FlashInfer tests...")

        test_grouped_gemm_correctness("float8_e4m3fn", "float8_e4m3fn", "bfloat16", 1, 128, 128, "K", 1,
        test_case={"batch_size": 4, "m_sizes": [128, 256, 192, 320], "n": 512, "k": 1024})
        print("Basic correctness test passed")
        
        
        print("All tests completed!")
    else:
        print("FlashInfer or CUTLASS SM90+ not available, skipping tests")